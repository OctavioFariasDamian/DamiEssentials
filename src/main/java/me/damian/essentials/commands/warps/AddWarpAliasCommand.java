...
public class AddWarpAliasCommand implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(args.length < 2){
            sendMessageWithPrefix(sender, "&cEl uso del comando es: /addwarpalias <nombre> <alias>", WarpsManager.prefix);
            return false;
        }
        ...
        warp.getAliases().add(alias);

        sendMessageWithPrefix(sender, "&fAñadiste un alias al warp &e"+name+"&f: &a"+alias, WarpsManager.prefix);
        warp.save();
        return false;
    }
